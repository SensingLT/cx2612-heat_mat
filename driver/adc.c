...
#define AD_TO_RES(ad) (100 * (ad) / (4096 - ad))

//PC3-ADC_IN10
#define NTC0_PORT AFIOC
#define NTC0_PIN GPIO_Pin_3
#define NTC0_ADC_CHANNEL ADC_Channel_10
#define NTC0_ADC_SCAN_CHANNEL ADC_ScanChannel_0

//PC4-ADC_IN7
#define NTC1_PORT AFIOC
#define NTC1_PIN GPIO_Pin_4
#define NTC1_ADC_CHANNEL ADC_Channel_7
#define NTC1_ADC_SCAN_CHANNEL ADC_ScanChannel_1

//PC5-ADC_IN9
#define CS_PORT AFIOC
#define CS_PIN  GPIO_Pin_5
#define CS_ADC_CHANNEL ADC_Channel_9
#define CS_ADC_SCAN_CHANNEL ADC_ScanChannel_2

//PA1-ADC_IN1
#define PH0_PORT AFIOA
#define PH0_PIN  GPIO_Pin_1
#define PH0_ADC_CHANNEL ADC_Channel_1
#define PH0_ADC_SCAN_CHANNEL ADC_ScanChannel_3

//PA2-ADC_IN0
#define PH1_PORT AFIOA
#define PH1_PIN  GPIO_Pin_2
#define PH1_ADC_CHANNEL ADC_Channel_0
#define PH1_ADC_SCAN_CHANNEL ADC_ScanChannel_4
...
{
    //config gpio
    GPIO_AnalogRemapConfig(NTC0_PORT, NTC0_PIN, ENABLE);
    GPIO_AnalogRemapConfig(NTC1_PORT, NTC1_PIN, ENABLE);
    GPIO_AnalogRemapConfig(CS_PORT, CS_PIN, ENABLE);
    ...
    adcCfg.ADC_TimerTriggerSource = ADC_TimerTriggerSource_TIM3ADC; //定时源触发选择TIM3事件
    ...
    ADC_ScanChannelConfig(ADC, PH0_ADC_CHANNEL, 3);
	ADC_ScanChannelConfig(ADC, PH1_ADC_CHANNEL, 4);
    ADC_ScanChannelNumberConfig(ADC, 5);
...
void ADC_Handler(void) {
    if (ADC_GetFlagStatus(ADC, ADC_FLAG_EOS)) {
		...
		gAdcCtrl.adc4_ph0 = ADC_GetScanData(ADC, PH0_ADC_SCAN_CHANNEL) >> 3;
		gAdcCtrl.adc5_ph1 = ADC_GetScanData(ADC, PH1_ADC_SCAN_CHANNEL) >> 3;
...
uint32_t Adc_csCurrentGet(uint32_t *current) {
	...
	//DBG_LN("gAdcCtrl.adc3_csVol : %d",gAdcCtrl.adc3_csVol);	
	//DBG_LN("voltage_mV : %d",voltage_mV);
    // 电流：mA，系数 3.7 = 37/10
...
uint32_t Adc_phGet(uint16_t *r1, uint16_t *r2) {
	*r1 = AD_TO_RES(gAdcCtrl.adc4_ph0);
	*r2 = AD_TO_RES(gAdcCtrl.adc5_ph1);
	if(*r1 < 20 && *r2 <20){
		DBG_LN("two people sit on");
	}
	else if(*r1 < 20){
		DBG_LN("front people sit on");
	}
	else if(*r2 < 20){
		DBG_LN("rear people sit on");
	}
    return gAdcCtrl.update_tick;
}
